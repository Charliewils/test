...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    for col in range(max_cols-num_connect+1):
        account=0
        for i in range(num_connect):
            if get_piece(board,row,col+i) == player:
                account += 1
            else:
                account=0
            if account == num_connect:
                return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    False
    """
    for row in range(max_rows-num_connect+1):
        account=0
        for i in range(num_connect):
